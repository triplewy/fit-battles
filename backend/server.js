...
conn.query('CREATE TABLE IF NOT EXISTS users (userId INTEGER AUTO_INCREMENT PRIMARY KEY, profileName VARCHAR(255) NOT NULL UNIQUE, location TEXT, createdDate DATETIME DEFAULT CURRENT_TIMESTAMP NOT NULL)')
...
conn.query('CREATE TABLE IF NOT EXISTS posts (mediaId INTEGER AUTO_INCREMENT PRIMARY KEY, userId INTEGER NOT NULL, imageUrl VARCHAR(255) NOT NULL, profileName TEXT NOT NULL, wins INTEGER DEFAULT 0, matches INTEGER DEFAULT 0, dateTime DATETIME DEFAULT CURRENT_TIMESTAMP NOT NULL, FOREIGN KEY (userId) REFERENCES users(userId));')
conn.query('CREATE TABLE IF NOT EXISTS votes (voteId INTEGER AUTO_INCREMENT PRIMARY KEY, userId INTEGER, winMediaId INTEGER NOT NULL, winUserId INTEGER NOT NULL, lossMediaId INTEGER, lossUserId INTEGER, dateTime DATETIME DEFAULT CURRENT_TIMESTAMP NOT NULL, FOREIGN KEY (winMediaId) REFERENCES posts(mediaId) ON DELETE CASCADE, FOREIGN KEY (lossMediaId) REFERENCES posts(mediaId) ON DELETE CASCADE, ' +
'FOREIGN KEY (winUserId) REFERENCES users(userId), FOREIGN KEY (lossUserId) REFERENCES users(userId), FOREIGN KEY (userId) REFERENCES users(userId));')
